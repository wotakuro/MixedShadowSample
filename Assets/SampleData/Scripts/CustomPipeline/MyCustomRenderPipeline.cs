...
namespace App
{
    ...
    {
        private readonly MyCustomRenderPipelineAsset m_Asset;
        ...
        public MyCustomRenderPipeline(MyCustomRenderPipelineAsset asset)
        {
            ...
            {
                m_ShadowMapRT = new RenderTexture(shadowmapDescriptor);
...
//            SceneViewDrawMode.SetupDrawMode();
        ...
        CullResults m_CullResultsWithSelfShadow;
        CullResults m_CullResultsWithoutSelfShadow;
        ...
        public override void Render(ScriptableRenderContext context, Camera[] cameras)
        {
            ...
            foreach (Camera camera in cameras)
            {
                ...
                if (!CullResults.GetCullingParameters(m_CurrCamera, stereoEnabled, out cullingParameters))
                    continue;
                int originalCullingMask = cullingParameters.cullingMask;
                ...
                // 影の描画
                cullingParameters.cullingMask = originalCullingMask;

                cullingParameters.cullingMask = ~(m_Asset.NoSelfShadowLayerMask);
                CullResults.Cull(ref cullingParameters, context, ref m_CullResultsWithSelfShadow);
                bool shadows = ShadowPass(visibleLights, ref context, ref lightData, ref m_CullResultsWithSelfShadow, ClearFlag.Depth);
                ...
                if (LightweightUtils.HasFlag(frameRenderingConfiguration, FrameRenderingConfiguration.DepthPrePass))
                    DepthPass(ref context, frameRenderingConfiguration,ref m_CullResults);

                // self shadowなしのLayerだけをまず描画します
                cullingParameters.cullingMask = (m_Asset.NoSelfShadowLayerMask) & originalCullingMask;
                CullResults.Cull(ref cullingParameters, context, ref m_CullResultsWithoutSelfShadow);
                ...
                ForwardPass(visibleLights, frameRenderingConfiguration, ref context, ref lightData, stereoEnabled,false,ref m_CullResultsWithoutSelfShadow);


                //
                var oldShadomapSampleType = lightData.shadowMapSampleType;
                // submit しないと　Context内にShadowがうまく描画できませんので・・・
                // CastShadowの対象一覧が正しく処理されません
                context.Submit();
                // キャラクターの影を描画します
                InitializeLightData(visibleLights, out lightData);
                ShadowPass(m_CullResultsWithoutSelfShadow.visibleLights, ref context, ref lightData, ref m_CullResultsWithoutSelfShadow, ClearFlag.None);
                // SelfShadowなしのやつがいなかった時の対策
                if (lightData.shadowMapSampleType == LightShadows.None)
                {
                    lightData.shadowMapSampleType = oldShadomapSampleType;
                }
                ...
                if (shadows && m_ShadowSettings.screenSpace)
                    ShadowCollectPass(visibleLights, ref context, ref lightData, frameRenderingConfiguration);
                ForwardPass(visibleLights, frameRenderingConfiguration, ref context, ref lightData, stereoEnabled,true,ref m_CullResultsWithSelfShadow);
        ...
        private bool ShadowPass(List<VisibleLight> visibleLights, ref ScriptableRenderContext context, ref LightData lightData,ref CullResults cullResults,ClearFlag clearFlag)
        {
//            m_ShadowMapRT = null;
            if (m_Asset.AreShadowsEnabled() && lightData.mainLightIndex != -1 && visibleLights.Count > (lightData.mainLightIndex))
            {
                ...
                if (mainLight.light.shadows != LightShadows.None)
                {
                    ...
                    bool shadowsRendered = RenderShadows(ref cullResults, ref mainLight, shadowOriginalIndex, ref context, clearFlag);
                    if (shadowsRendered)
                    {
        ...
        private void DepthPass(ref ScriptableRenderContext context, FrameRenderingConfiguration frameRenderingConfiguration,ref CullResults cullResults)
        {
        ...
        private void ForwardPass(List<VisibleLight> visibleLights, FrameRenderingConfiguration frameRenderingConfiguration, ref ScriptableRenderContext context, ref LightData lightData, bool stereoEnabled,bool forceClearNone,ref CullResults cullResults)
        {
            ...
            BeginForwardRendering(ref context, frameRenderingConfiguration,forceClearNone);
            RenderOpaques(ref context, rendererSettings , ref cullResults);
            AfterOpaque(ref context, frameRenderingConfiguration);
            RenderTransparents(ref context, rendererSettings,ref cullResults);
            AfterTransparent(ref context, frameRenderingConfiguration);
            EndForwardRendering(ref context, frameRenderingConfiguration);
        }

        private void RenderOpaques(ref ScriptableRenderContext context, RendererConfiguration settings,ref CullResults cullResults)
        {
            ...
            RenderObjectsWithError(ref context, opaqueFilterSettings, SortFlags.None,ref cullResults);
        ...
        private void RenderTransparents(ref ScriptableRenderContext context, RendererConfiguration config,ref CullResults cullResults)
        {
            ...
            // Render objects that did not match any shader pass with error shader
            RenderObjectsWithError(ref context, transparentFilterSettings, SortFlags.None,ref cullResults);
        }
        ...
        private void RenderObjectsWithError(ref ScriptableRenderContext context, FilterRenderersSettings filterSettings, SortFlags sortFlags,ref CullResults cullResults)
        {
        ...
        private void SetupShaderConstants(List<VisibleLight> visibleLights, ref ScriptableRenderContext context, ref LightData lightData,ref CullResults cullResults)
        {
            ...
            SetupShaderLightConstants(cmd, visibleLights, ref lightData,ref cullResults);
        ...
        private void SetupShaderLightConstants(CommandBuffer cmd, List<VisibleLight> lights, ref LightData lightData,ref CullResults cullResults)
        {
        ...
        private bool RenderShadows(ref CullResults cullResults, ref VisibleLight shadowLight, int shadowLightIndex, ref ScriptableRenderContext context,ClearFlag clearFlag)
        {
            ...
            CoreUtils.SetRenderTarget(cmd, m_ShadowMapRT, clearFlag, CoreUtils.ConvertSRGBToActiveColorSpace(m_CurrCamera.backgroundColor));
        ...
        private void BeginForwardRendering(ref ScriptableRenderContext context, FrameRenderingConfiguration renderingConfig, bool forceClearNone)
        {
